...
module.exports = function(client) {
  ...
  var run = function(args, cb) {
    ...
    if (args['m']) {  // switch input to mobius
      setTimeout(function() {
        marantz.invoke('mobius');
      }, 200);
    }
    ...
    if (args['f']) {
      mpc.run(['clear'], client.host, client.port);
      _cb = cb
      (function() {
        cb = function() {
          mpc.run(['play'], client.host, client.port, _cb);
        });
      })();
    }
